...
\[I=\frac{dE}{dA_\perp dt}.\]
Le unit\`a di misura sono $[I]=\mathrm{J}\ \mathrm{m}^{-2}\ \mathrm{s}\ii$.
\end{definition}

\begin{definition}[Radianza]
La \textbf{radianza} di un'onda per un certo angolo solido \`e
\[I_\Omega=\frac{dE}{dA_\perp dtd\Omega}=\dd \Omega E.\]
Le unit\`a di misura sono le stesse dell'intensit\`a.
\end{definition}

\begin{definition}[Emittanza]
L'\textbf{emittanza} come la potenza emessa da una supeficie per unit\`a di superficie. Si indica con $M$.
\end{definition}

\begin{definition}[Irradianza]
